  ...
  return (
    <div className="container animate-fade-in" style={{ padding: '2rem 1.5rem', maxWidth: '900px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
        ...
        <h2 style={{ fontFamily: 'var(--font-serif)' }}>Your Shopping Cart</h2>
      </div>
      ...
      {cartItems.length === 0 ? (
        ...
        <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
          <div style={{ flex: '1 1 500px', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {cartItems.map(item => (
              <div key={item.id} className="glass-panel" style={{ display: 'flex', padding: '1.5rem', gap: '1.5rem', alignItems: 'center' }}>
                <div style={{ width: '60px', height: '80px', background: 'var(--color-bg-secondary)', borderRadius: '4px', flexShrink: 0, overflow: 'hidden' }}>
                    {item.book.image_url && <img src={item.book.image_url} alt={item.book.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
                </div>
                <div style={{ flex: 1 }}>
                  <h4 style={{ fontFamily: 'var(--font-serif)' }}>{item.book.title}</h4>
                  <p style={{ color: 'var(--color-text-secondary)', fontSize: '0.9rem', marginBottom: '0.5rem' }}>{item.book.author}</p>
                  
                  <div style={{ display: 'flex', alignItems: 'center', background: 'var(--color-bg-secondary)', borderRadius: 'var(--radius-full)', border: '1px solid var(--color-glass-border)', overflow: 'hidden', width: 'fit-content' }}>
                    <button onClick={() => updateQuantity(item.id, item.quantity - 1)} style={{ background: 'transparent', border: 'none', color: 'var(--color-text-primary)', padding: '0.25rem 0.75rem', cursor: 'pointer' }}>
                      <Minus size={14} />
                    </button>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <button onClick={() => removeFromCart(item.id)} className="btn-icon" style={{ background: 'rgba(220, 38, 38, 0.06)', color: '#dc2626', borderColor: 'transparent' }}>
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
          ...
          <div className="glass-panel" style={{ flex: '1 1 300px', padding: '2rem', height: 'fit-content' }}>
            <h3 style={{ marginBottom: '1.5rem', borderBottom: '1px solid var(--color-glass-border)', paddingBottom: '1rem', fontFamily: 'var(--font-serif)' }}>Order Summary</h3>
