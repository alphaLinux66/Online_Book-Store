...
export default function StoreOwnerDashboard() {
  ...
  return (
    <div className="container" style={{ padding: '2rem 1rem' }}>
      <h1 className="text-gradient">Supplier B2B Portal</h1>
      ...
      <div style={{ marginTop: '2rem' }}>
          ...
          <div>
            ...
            {showBookForm && (
              <div className="glass-panel animate-fade-in" style={{ padding: '1.5rem', marginBottom: '2rem' }}>
                 <h3>{editingBook ? 'Edit Book' : 'List Book for B2B Purchasing'}</h3>
            ...
            <div className="glass-panel" style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                ...
                <tbody>
                  {books.length === 0 ? (
                      <tr><td colSpan="4" style={{ padding: '1rem', textAlign: 'center' }}>No wholesale listings yet.</td></tr>
                  ) : books.map(b => (
                    <tr key={b.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                      ...
                      <td style={{ padding: '1rem', color: '#10b981', fontWeight: 'bold' }}>₹{b.wholesale_price}</td>
        ...
        {activeTab === 'orders' && (
          <div>
            <h2 style={{ marginBottom: '1.5rem' }}>Fulfillment Queue</h2>
            {orders.length === 0 ? (
                <div className="glass-panel" style={{ padding: '2rem', textAlign: 'center', color: 'var(--color-text-secondary)' }}>No B2B orders pending.</div>
            ) : orders.map(order => (
                <div key={order.id} className="glass-panel" style={{ padding: '1.5rem', marginBottom: '1rem', borderLeft: order.status === 'Pending' ? '4px solid #f59e0b' : (order.status === 'Shipped' ? '4px solid #3b82f6' : '4px solid #10b981') }}>
                   <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                       <div>
                           <h3 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                               Order #{order.id} 
                               <span style={{ fontSize: '0.8rem', padding: '0.2rem 0.6rem', borderRadius: '12px', background: 'rgba(255,255,255,0.1)' }}>{order.status}</span>
                           </h3>
                           <p style={{ margin: '0.5rem 0 0 0', color: 'var(--color-text-secondary)', fontSize: '0.9rem' }}>Placed by Retail Admin: <strong>{order.admin_name}</strong></p>
                           <p style={{ margin: '0', color: 'var(--color-text-secondary)', fontSize: '0.9rem' }}>Invoice Total: <span style={{ color: '#10b981', fontWeight: 'bold' }}>₹{order.total_amount}</span></p>
                       </div>
                       ...
                       <div style={{ display: 'flex', gap: '0.5rem' }}>
                           ...
                           {order.status === 'Shipped' && (
                               <button className="btn" style={{ background: '#10b981', color: 'white' }} onClick={() => handleUpdateStatus(order.id, 'Delivered')}><CheckCircle size={16}/> Confirm Delivery to Retailer</button>
                           )}
                       </div>
                   </div>
                   ...
                   <div style={{ background: 'rgba(0,0,0,0.2)', padding: '1rem', borderRadius: '8px' }}>
                       ...
                       {order.items.map(item => (
                           <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.5rem 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
